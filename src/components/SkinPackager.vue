...
function getSkinType(imageUrl: string): Promise<"custom" | "customSlim"> {
  return new Promise(async (resolve, reject) => {
    ...
    img.onload = function () {
      ...
      console.log(ctx.getImageData(50 * resolutionMultiplier, 16 * resolutionMultiplier, 1 * resolutionMultiplier, 1 * resolutionMultiplier));
  ...
  <div class="flex-grow-1">
    ...
    <v-row>
      <v-col>
        ...
        <v-file-input variant="outlined" label="File input" multiple @change="displayskins"></v-file-input>        
      </v-col>
    </v-row><v-row>
      <v-col cols="12">
        <draggable class="d-flex align-center justify-center row flex-wrap" :list="skins" group="people"
          @start="drag = true" @end="drag = false" item-key="element.url">
          <template #item="{ element }">
            <v-card class="flex-grow-0 ma-5">
              <v-card-title>
